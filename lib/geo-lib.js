...
'use strict';
 ...
 var geoVincenty = require('./geo-vincenty');
 ...
 */
GeoLib.prototype.distance = function (opts) {
    let opt;
    ...
    if (opt.method === 'vincenty') {
        method = opt.method;
        dist = geoVincenty.distance(
            opt.p1,
            opt.p2
        );
        dist = (dist / 1000).toFixed(2);
    } else {
        dist = distanceHaversine(opt.p1, opt.p2);
    }
    let result = {
        distance: parseFloat(dist), // 1447.35,
        unit: 'km',
        method: method
    };
    if (opt.timeUsed) {
 ...
 */
GeoLib.prototype.pointInsidePolygon = function (inputPoint, inputPolygon) {
    let polygon = [],
        point = {};
 ...
 */
GeoLib.prototype.linesIntersect = function (inputLine1p1, inputLine1p2, inputLine2p1, inputLine2p2) {
    let line1p1, line1p2,
        line2p1, line2p2;
 ...
 */
GeoLib.prototype.polygonOverlapsPolygon = function (inputPolygonA, inputPolygonB) {
    let polygonA = [],
        polygonB = [];
    // Check if we have an array of arrays as input. If so make it an
    // array of objects.
